                            ...
                            <div class="mx-auto max-w-2xl lg:max-w-none">
                                <div class="grid grid-cols-1 sm:grid-cols-2">
                                    <NuxtLink
                                        ...
                                        v-for="item in menu"
                                        class="group relative isolate -mx-6 bg-black px-6 py-10 before:w-px even:mt-px hover:text-black sm:mx-0 sm:px-0 sm:py-16 sm:odd:pr-16 sm:even:mt-0 sm:even:pl-16"
                                        :to="localePath(item.route)"
                                        >{{ $t(item.button)
                                        }}<span
                                            class="bg-lemon-900 absolute inset-y-0 -z-10 w-screen opacity-0 transition group-odd:right-0 group-even:left-0 group-hover:opacity-100"></span
                                    ></NuxtLink>
                                </div>
                            </div>
                        </div>
